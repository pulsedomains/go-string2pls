...
import (
	...
	"github.com/pulsedomains/go-string2pls"
)
...
func TestRoundTripWithSmallValue(t *testing.T) {
	...
	third := string2pls.WeiToString(second, false)
	assert.Equal(t, first, third, "Did not receive expected result")
	fourth := string2pls.WeiToString(second, true)
...
func TestRoundTripWithNormalValue(t *testing.T) {
	...
	third := string2pls.WeiToString(second, true)
	assert.Equal(t, first, third, "Did not receive expected result")
}

func ExampleUnitToMultiplier() {
	multiplier, err := string2pls.UnitToMultiplier("ether")
	if err != nil {
		return
	}
...
func TestStringToWei(t *testing.T) {
	...
	for i, test := range tests {
		result, err := string2pls.StringToWei(test.input)
		if err != nil {
...
func TestWeiToString(t *testing.T) {
	...
	for i, test := range tests {
		result := string2pls.WeiToString(test.input, test.canonical)
		assert.Equal(t, test.result, result, fmt.Sprintf("Incorrect value at test %d", i))
	}
}
...
func TestGWeiToString(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := string2pls.GWeiToString(test.input, test.canonical)
...
func TestStringToGWei(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result, err := string2pls.StringToGWei(test.input)
			if test.err != nil {
...
func TestWeiToGWeiString(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := string2pls.WeiToGWeiString(test.input)
			require.Equal(t, test.result, result)
		})
	}
}
